...
def load_flats(data_dir, bias_level=300):
    """
    ...
    flat_data = [fits.getdata(f) - bias_level for f in flats]
...
def remove_cosmic(input_array):
    ...
    """
    _, clean_array = astroscrappy.detect_cosmics(input_array)
    return clean_array
...
def create_master_flat(
    ...
    flat_data = [img[1:-1, 1:-1] for img, keep in zip(flat_data, keep_mask) if keep]
    ...
    median_array /= np.max(median_array)
