...
def process_light(light, master_flat, output_wcs, output_shape, ra, dec):
    try:
        light_vals = (
            fits.getdata(light).astype("float32")[1:-1, 1:-1] - 300.0
        )  # Subtract fixed bias
        light_vals = light_vals / master_flat
        ...
        light_vals = calculate_reprojection(
            source_hdus=(light_vals, fits.getheader(wcs_solved_path)),
...
def create_stack(filter_, pos, data_dir, output_dir, flat_path, target_name):
    """
    ...
    if len(valid_light_paths) > 0:
        ...
        light_data = Parallel(n_jobs=4, backend="loky")(
            delayed(process_light)(
                light, master_flat, output_wcs, output_shape, ra, dec
            )
